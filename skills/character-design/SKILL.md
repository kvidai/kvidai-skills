...
1. Start from routed endpoint IDs.
   ...
   ```bash
   kvidai docs "consistent character generation" --json
   kvidai models "image editing character consistency" --json
   ```
...
3. Upload references.
   ...
   kvidai upload ./costume-reference.png --json
   ```
...
- Premium but cheaper image option: use `kvid-ai/nano-banana-pro` or
  `kvid-ai/nano-banana-2`.
- Fast exploratory drafts: use `kvid-ai/flux-2/klein/9b`.
...
- Outfit variations and character edits: use `kvid-ai/nano-banana-pro/edit`,
  then `openai/gpt-image-2/edit`, then
  `kvid-ai/bytedance/seedream/v5/lite/edit`.
...
- Talking avatar or lip-sync: use `veed/fabric-1.0`,
  `veed/fabric-1.0/text`, or `kvid-ai/creatify/aurora`.
