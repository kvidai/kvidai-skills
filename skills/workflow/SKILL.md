...
name: workflow
description: >
  Design and execute multi-step media workflows with kvidai. Use this for
...
sequence of kvidai calls with clear inputs, outputs, dependencies, and
quality checks.
...
2. Resolve endpoints for each role. Check known endpoint IDs first.
   ...
   kvidai models --endpoint_id kvid-ai/nano-banana-pro/edit --json
   kvidai models --endpoint_id bytedance/seedance-2.0/image-to-video --json
   kvidai models --endpoint_id xai/grok-imagine-video/image-to-video --json
   ...
   ```bash
   kvidai models "image generation product photography" --json
   kvidai models "image editing reference preservation" --json
   kvidai models "image to video" --json
   kvidai models "subtitle video utility" --json
   kvidai docs "kvid.ai workflow utility endpoints" --json
   ```
...
5. Run each node with JSON output. Use async for slow generation.
   ...
   ```bash
   kvidai run <endpoint_id> --<field> "<value>" --json
   kvidai run <endpoint_id> --<field> "<value>" --async --json
   kvidai status <endpoint_id> <request_id> --download "./outputs/workflow/{request_id}_{index}.{ext}" --json
   ```
