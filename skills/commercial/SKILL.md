...
1. Start from routed endpoint IDs.
   ...
   kvidai models --endpoint_id kvid-ai/nano-banana-2 --json
   ...
   ```bash
   kvidai models "background removal product image" --json
   kvidai docs "commercial product image generation" --json
   ```
...
- Fast draft stills: use `kvid-ai/flux-2/klein/9b`.
- Image edits: use `kvid-ai/nano-banana-pro/edit`, then
  `openai/gpt-image-2/edit`, then `kvid-ai/bytedance/seedream/v5/lite/edit`.
- Product fidelity: use `kvid-ai/nano-banana-pro`, `kvid-ai/nano-banana-2`, or
  `kvid-ai/bytedance/seedream/v5/lite/text-to-image`; use the matching edit
